...
using SellUi = DreamPoeBot.Loki.Game.LokiPoe.InGameState.TradeUi;
...
namespace SimpleBot.Extensions
{
    public static class TownNpcs
    {
        ...
        public static TownNpc GetSellVendor()
        {
            ...
            {
                var npc = LokiPoe.ObjectManager.Objects.Closest<Npc>(n => n.IsTargetable && n.Name != "Ange");
                if (npc == null)
                {
            ...
            if (area.IsTown)
            {
                GlobalLog.Error("[GetSellVendor] Cannot sell items in town. Use hideout instead.");
                return null;
            }
        ...
        public static async Task<bool> SellItems(List<Vector2i> itemPositions)
        {
            ...
            itemPositions.Sort(new Vector2iComparer());
    ...
    public class TownNpc
    {
        ...
        public async Task<bool> OpenPurchasePanel()
        {
            ...
            if (!await Wait.For(() => LokiPoe.InGameState.TradeUi.IsOpened, "purchase panel opening"))
                return false;
