    ...
    public class SortInventoryTask : ITask
    {
        public async Task<bool> Run()
        {
            ...
            while (true)
            {
                ...
                if (cursorItem != null)
                {
                    ...
                    if (!await MoveCursorItem(pos.AsVector))
                    {
        ...
        private static ItemToMove GetItemToMove()
        {
            foreach (var item in Inventories.InventoryItems)
            {
                ...
                if (pos != null)
                {
                    return new ItemToMove(item.Name, item.LocationTopLeft, pos.AsVector);
                }
            }
            return null;
        }
        ...
        private static Position GetMovePosForInventoryItem(Item item)
        {
            ...
            if (currency != null && HasFixedPosition(currency))
            {
                ...
                if (destination.AsVector != item.LocationTopLeft)
                {
                    if (OccupiedBySameItem(itemName, destination.AsVector))
                    {
        ...
        private static Position GetMovePosLessThanCurrent(Item item)
        {
            ...
            return Position.Comparer.Instance.Compare(pos.AsVector, item.LocationTopLeft) < 0 ? pos : null;
        }
